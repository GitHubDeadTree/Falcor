...
# 正则表达式，匹配 "name": "B" 模式的字符串 (精确匹配)
pattern = re.compile(r'"name"\s*:\s*"B"')
...
for i, line in enumerate(lines):
    ...
    if match:
        # 增加计数器
        if "B" not in node_counters:
            node_counters["B"] = 0
        node_counters["B"] += 1

        # 创建新名称，添加下划线和后缀
        new_name = f"B_{node_counters['B']}"

        # 替换行中的名称
        new_line = line.replace('"name": "B"', f'"name": "{new_name}"')
        modified_lines.append(new_line)
        print(f"Line {i+1}: 修改 'B' 为 '{new_name}'")
    else:
